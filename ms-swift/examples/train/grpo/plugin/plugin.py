...
from map_util import compute_map_score
from video_clip_scheduler import _get_score
logger = get_logger()
"""
...
def clue_accuracy(identified_frames_score_list: List[Dict[int, int]],
                  frame_score: List[int],
                  round_frms: List[List[int]]) -> float:
    if not identified_frames_score_list:
        return None
    
    total_acc = 0.0
    mask=0
    for pred_dict,round_frm in zip(identified_frames_score_list,round_frms):
        temp_pred_dict = {int(k): int(v) for k, v in pred_dict.items()}
        pred_dict=temp_pred_dict
        pred_ge4: Set[int] = {int(k) for k, v in pred_dict.items() if v >= 4}
        pred_ge3: Set[int] = {int(k) for k, v in pred_dict.items() if v >= 3}

        gt_ge4: Set[int] = {int(i) for i, score in enumerate(frame_score)
                             if score >= 4 and i in round_frm and i < len(frame_score)}
        gt_ge3: Set[int] = {int(i) for i, score in enumerate(frame_score)
                             if score >= 3 and i in round_frm and i < len(frame_score)}
        
        acc = 0.0
        if gt_ge4 and pred_ge4:
            union = gt_ge4 | pred_ge4
            inter = gt_ge4 & pred_ge4
            acc = len(inter) / len(union) if union else 0.0
        elif gt_ge3 or pred_ge3:
            union = gt_ge3 | pred_ge3
            inter = gt_ge3 & pred_ge3
            acc = len(inter) / len(union) if union else 0.0
        else:
            mask=mask+1
            acc = 0.0 
        total_acc += acc
    if mask==len(identified_frames_score_list):
        clue_acc=0.0
    else:
        clue_acc=total_acc/(len(identified_frames_score_list)-mask)
    return clue_acc 
def action_accuracy(new_frames_idx_list: List[List[int]],
                    frame_score: List[int],
                    round_frms: List[List[int]]) -> float:

    if not new_frames_idx_list or len(round_frms)==1:
        return None
    history_set = set()
    total_acc = 0.0
    for idx, pred in enumerate(new_frames_idx_list):
        temp_pred = sorted([int(k) for k in pred])
        pred=temp_pred
        round_frm=round_frms[idx]
        history_set.update(round_frm)
        gt_ge4 = {i for i, score in enumerate(frame_score) if score >= 4 and i not in history_set}
        gt_ge3 = {i for i, score in enumerate(frame_score) if score >= 3 and i not in history_set}
        pred_ge4 = {k for k in pred if k < len(frame_score) and frame_score[k] >= 4}
        pred_ge3 = {k for k in pred if k < len(frame_score) and frame_score[k] >= 3}
        acc = 0.0
        if gt_ge4 and pred_ge4:
            inter = pred_ge4 & gt_ge4
            acc = len(inter) / min(len(gt_ge4), 8) if gt_ge4 else 0.0
        elif gt_ge3 and pred_ge3:
            inter = pred_ge3 & gt_ge3
            acc = len(inter) / min(len(gt_ge3), 8) if gt_ge3 else 0.0
        total_acc += acc

    return total_acc / len(new_frames_idx_list)
def get_info(info,last_completion,round_frm,fps,start_second=5.0):
    rounds=info['num_turns']
    identified_frames_score_list=[]
    new_frames_idx_list=[]
    for idx in range(1, rounds + 1):
        info_i=info.get(f"round{idx}",None)
        if not info_i:
            break
        identified_frames_score,new_frames_idx=info_i.get('identified_frames_score',None),info_i.get('new_frames_idx',None)
        if identified_frames_score:
            identified_frames_score_list.append(identified_frames_score)
        if new_frames_idx:
            new_frames_idx_list.append(new_frames_idx)
    frm_score = _get_score(last_completion,round_frm,fps,start_second)
    if len(frm_score)>0:
        identified_frames_score_list.append(frm_score)
    return identified_frames_score_list,new_frames_idx_list
def Clue_Action_Factor(identified_frames_score_list,new_frames_idx_list,frame_score,round_frms,outcome_acc):
    clue_acc,action_acc=None,None
    if identified_frames_score_list is not None and len(identified_frames_score_list)>0 and frame_score:
        clue_acc=clue_accuracy(identified_frames_score_list,frame_score,round_frms)
    if new_frames_idx_list is not None and len(new_frames_idx_list)>0 and frame_score:
        action_acc=action_accuracy(new_frames_idx_list,frame_score,round_frms)
    if clue_acc is None:
        clue_acc=0.0
...
class MultiturnFormat(ORM):
    def __call__(self, completions: List[str], **kwargs) -> List[float]:
        rewards = []
        for content in completions:
            m = REWARD_ORDER.search(content)
            if not m:
                rewards.append(0.0)
                continue
            score_txt, think_txt, clip_txt, ans_txt = [g.strip() for g in m.groups()]
            has_clip = bool(clip_txt)
            has_ans  = bool(ans_txt)  
...
class McAccuracy(ORM):
    def __call__(self, completions, solution, task, frame_score=None, cognitive_map=None,round_frame_id=None, **kwargs) -> List[float]:
        rewards=[]
        infos = kwargs.get('rollout_infos', [{"num_turns":1}]*len(completions))
        fps_list=kwargs.get("fps", [0.2]*len(completions))
        start_second_list = kwargs.get("start_second",[5.0]*len(completions))

        for content, sol, t, frm_score, info, fps, start_second,round_frm_id in zip(completions, solution, task, frame_score, infos, fps_list, start_second_list,round_frame_id):
                if t !="multiple choice":
                    ...
                    if "<answer>" in response:
                        response=extract_answer(response)
                    if "<answer>" in sol:
                        sol=extract_answer(sol)
                    if sol is None or response is None:
                        reward=0.0
                    else:
                        try:
                            reward = 1.0 if response.strip() == sol.strip() else 0.0
                        except:
                            reward=0.0
                    if reward>0:
                        extra_data_info=info.get("extra_data_info",None)
                        if extra_data_info is not None:
                            round_frm_id=info['extra_data_info']['round_frame_id']
                        try:
                            identified_frames_score_list,new_frames_idx_list=get_info(info,content,round_frm_id[-1],fps,start_second)
                            clue_action_factor=Clue_Action_Factor(identified_frames_score_list,new_frames_idx_list,frm_score,round_frm_id,reward)
                            reward=reward+clue_action_factor
                        except Exception as e:
                            import traceback
                            print(f"[ERROR] in McAccuracy {type(e).__name__}: {e}")
                            traceback.print_exc() 
                            pass
...
class FreeformAccuracy(ORM):
    ...
    def __call__(self, completions, solution, task, frame_score,round_frame_id, **kwargs) -> List[float]:
        rewards = []
        infos = kwargs.get('rollout_infos', [{"num_turns":1}]*len(completions))
        fps_list=kwargs.get("fps", [0.2]*len(completions))
        start_second_list = kwargs.get("start_second",[5.0]*len(completions))
        for content, sol, t, frm_score, info, fps, start_second, round_frm_id in zip(completions, solution, task, frame_score, infos, fps_list, start_second_list,round_frame_id):
            if t !="free-form":
                ...
                if sol is None or response is None:
                    ...
                    try:
                        score = FreeformAccuracy.compute_rouge_score(sol, response)
                ...
                if reward>0:
                    ...
                    try:
                        ...
                        import traceback
                        print(f"[ERROR] in FreeformAccuracy {type(e).__name__}: {e}")
                        traceback.print_exc() 
                        pass
            rewards.append(reward)
        return rewards
    ...
    def __call__(self, completions, solution, task, **kwargs) -> List[float]:
        ...
        for content, sol, t in zip(completions, solution, task):
            if t !="regression":
                ...
                if "<answer>" in sol:
                    sol=extract_answer(sol)
                if sol is None or content is None:
                    reward=0.0
                else:
                    gt_number = RegressionAccuracy.normalize_number(sol)
                    out_number = RegressionAccuracy.normalize_number(content)
        ...
        for content, sol, t in zip(completions, solution, task):
            if t !="OCR":
                ...
                if sol is None or content is None:
                    ...
                    try:
                        error_rate = OCRAccuracy.wer(sol, content)
                        reward = 1 - error_rate
                        reward = max(0.0, min(1.0, reward))
                    except:
                        print("computing OCRAccuracy error")
                        reward=0.0
            rewards.append(reward)
        return rewards
...
orms['numericalaccuracy'] = NumericalAccuracy
orms['freeformaccuracy'] = FreeformAccuracy
orms['regressionaccuracy'] = RegressionAccuracy
orms['ocraccuracy'] = OCRAccuracy
orms['mcaccuracy'] = McAccuracy
orms['multiturnformat'] = MultiturnFormat
"""
