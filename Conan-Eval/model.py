...
import os
import torch
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoProcessor, Qwen2_5_VLForConditionalGeneration, Qwen2VLForConditionalGeneration
...
BACKOFF_BASE   = 1.0   # 基础秒数
BACKOFF_FACTOR = 2.0   # 指数倍数
BACKOFF_MAX    = 60.0  # 最大休眠上限（秒）
BACKOFF_JITTER = 0.1   # 随机抖动比例（0.1=10%）
class Qwen2_5_VL:
    def __init__(self,
                 model_path: str,
                 gpu_ids: List[int],          
    ...
    def chat(self,
        ...
        max_retry=3) -> str:
        """
        与模型进行对话，支持图像输入和历史对话记录。
        
        参数:
            images: 图像列表，每个图像都是 PIL.Image.Image 对象。
            prompt: 用户输入的文本提示。
            history: 历史对话记录，格式为 List[Dict]，每个字典包含 "role" 和 "content"。
