...
def worker(rank: int,
    ...
    for ex in tqdm(tasks,desc=f"rank {rank}"):
        pred, prompt, messages, ok = bench.evaluate_example(model, ex, mode=mode, max_frames=max_frames,max_steps=max_steps)
        results.append(bench.make_result_record(ex, prompt, messages, pred, ok))
        if pred=="None":
            utils.log_sample(run_dir, rank, gpu_ids, ex, pred, ok)
    utils.save_rank_result(run_dir, rank, results)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--benchmark", required=True)
    parser.add_argument("--mode", choices=["step", "uniform"], required=True)
